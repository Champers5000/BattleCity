...
public class Tank {
	...
	public Tank(int x, int y, boolean good, Direction dir, TankClient tc, int player, String inputname) {
		this(x, y, good);
		this.direction = dir;
		this.tc = tc;
		this.player=player;
	...
	public void draw(Graphics g) {
		...
		if (good) {
			...
			g.drawString(name, x+(width-5*name.length())/2,y+50);
	...
	void move() {
		...
		if (!good) {
			...
			if (step == 0) {                  
				...
				int mod=r.nextInt(9);
				if (playertankaround()){
					if(x==tc.homeTank[0].x){ if(y>tc.homeTank[0].y) direction=directons[1];
					else if (y<tc.homeTank[0].y) direction=directons[3];
					}else if(y==tc.homeTank[0].y){ if(x>tc.homeTank[0].x) direction=directons[0];
					else if (x<tc.homeTank[0].x) direction=directons[2];
					}
					else{
	...
	public boolean playertankaround(){
		...
		if (this.live && a.intersects(tc.homeTank[0].getRect())) {
		return true;	
		}
		return false;	
	}
	...
	public void keyPressed(KeyEvent e) {  
		...
		case KeyEvent.VK_R:
			...
			*/
			tc.setVisible(false);
			TankClient abc=new TankClient(tc.names, tc.spawnTanks);
	...
	//fire bullets from player tanks
	public Bullets fire() {
		if (!live)
			return null;
		int x = this.x + Tank.width / 2 - Bullets.width / 2; 
		int y = this.y + Tank.length / 2 - Bullets.length / 2;
		Bullets m = new Bullets(x, y + 2, true, Kdirection, this.tc, player);
		tc.bullets.add(m);
		return m;
	}
